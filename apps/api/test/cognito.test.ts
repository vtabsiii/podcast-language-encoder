...
describe('cognito verifier', () => {
  ...
  test('accepts a valid token and maps the org claims', async () => {
    const token = sign({ sub: 'abc', iss: issuer, aud: 'client-1', exp: now + 60, iat: now, email: 'a@b.c', name: 'A', org_ids: '["o1","o2"]', role: 'producer' });
  ...
  test('accepts space-separated custom attributes as issued by the pre-token trigger fallback', async () => {
    const token = sign({ sub: 'abc', iss: issuer, client_id: 'client-1', token_use: 'access', exp: now + 60, 'custom:org_ids': 'o1 o2', 'custom:role': 'reviewer' });
    const claims = await verifier.verify(token);
    expect(claims.org_ids).toEqual(['o1', 'o2']);
  ...
  test('rejects bad signatures, wrong issuer, wrong audience, expiry, and unknown kid', async () => {
    ...
    await expect(verifier.verify(sign({ sub: 'abc', iss: 'https://evil', aud: 'client-1', exp: now + 60 }))).rejects.toThrow(/issuer/);
    await expect(verifier.verify(sign({ sub: 'abc', iss: issuer, aud: 'other', exp: now + 60 }))).rejects.toThrow(/audience/);
    await expect(verifier.verify(sign({ sub: 'abc', iss: issuer, aud: 'client-1', exp: now - 1 }))).rejects.toThrow(/expired/);
    await expect(verifier.verify(sign({ sub: 'abc', iss: issuer, aud: 'client-1', exp: now + 60 }, 'nope'))).rejects.toThrow(/signing key/);
  });
});
...
describe('database url composition from secret parts (ECS)', () => {
  test('composes owner and app urls when the URL variables are absent', () => {
    ...
    expect(cfg.DATABASE_URL).toBe('postgres://polycast:p%40ss%2Fword@db.internal:5432/polycast?sslmode=require');
    expect(cfg.DATABASE_APP_URL).toBe('postgres://polycast_app:app@db.internal:5432/polycast?sslmode=require');
  });
  test('explicit URLs win over parts', () => {
    const cfg = loadConfig({ NODE_ENV: 'test', DATABASE_URL: 'postgres://x', DB_HOST: 'h', DB_OWNER_USER: 'u', DB_OWNER_PASSWORD: 'p' });

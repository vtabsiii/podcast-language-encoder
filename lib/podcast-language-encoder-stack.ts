  ...
  constructor(scope: Construct, id: string, props: PodcastLanguageEncoderStackProps) {
    ...
    const startTranscription = new tasks.CallAwsService(this, 'StartTranscriptionJob', {
      ...
      parameters: {
        ...
        'OutputKey.$': "States.Format('transcribe-raw/{}/{}.json', $.episode.id, $$.Execution.Name)",
    ...
    startTranscription.addRetry({ errors: ['Transcribe.LimitExceededException'], interval: cdk.Duration.seconds(30), maxAttempts: 10, backoffRate: 2 });
    ...
    const translateAndSynthesize = new tasks.LambdaInvoke(this, 'TranslateAndSynthesize', {
      ...
      resultSelector: { 'tasks.$': '$.Payload.pollyTasks', 'manifestKey.$': '$.Payload.manifestKey' },
    ...
    const deriveEpisodeId = new sfn.Pass(this, 'DeriveEpisodeId', {
      parameters: {
        'id.$': "States.ArrayGetItem(States.StringSplit(States.ArrayGetItem(States.StringSplit($.detail.object.key, '/'), States.MathAdd(States.ArrayLength(States.StringSplit($.detail.object.key, '/')), -1)), '.'), 0)",
      },
      resultPath: '$.episode',
    });
    ...
    const definition = deriveEpisodeId
      ...
      .next(
        new sfn.Choice(this, 'IsTranscriptionComplete')
          .when(
            sfn.Condition.stringEquals('$.transcription.TranscriptionJob.TranscriptionJobStatus', 'COMPLETED'),
            translateAndSynthesize.next(waitForAllSpeech).next(new sfn.Succeed(this, 'EpisodeEncoded')),
          )
          .when(
            sfn.Condition.stringEquals('$.transcription.TranscriptionJob.TranscriptionJobStatus', 'FAILED'),
            transcriptionFailed,
          )

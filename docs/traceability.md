...
| FR-010 | `MockTranslationProvider` (local) or `services/media-worker/polycast_worker/providers/aws/translate.py` (Amazon Translate, batched ≤ 10,000 bytes, formality, terminology) / `providers/aws/bedrock.py` (Bedrock `converse`, segment ids preserved, previous segment + speaker as context) selected by `providers/registry.py`; `stages/translating.py`; `translation_versions` persistence | `tests/test_stage_units.py`; `tests/test_aws_translation.py` (recorded responses via Stubber); `tests/test_aws_e2e_loop.py`; `apps/api/test/slice.test.ts` | implemented (M1 mock; M3 adapters contract-tested against recorded responses, not yet run against live AWS) |
| FR-011 | `services/media-worker/polycast_worker/qc/entity_check.py` (URLs, numbers, capitalised multi-word names, glossary terms; locale-tolerant number matching) used by `stages/translating.py` (count only) and `providers/quality.py` `entity-preservation` → QC issues per segment; Bedrock system prompt demands verbatim entities | `tests/qc/test_entity_check.py`; `tests/test_quality.py`; `tests/test_aws_e2e_loop.py` | implemented (M3; glossary terms wired to an empty list until FR-015) |
...
| FR-013 | `services/media-worker/polycast_worker/providers/aws/bedrock.py` (`maxChars` per segment from `timingBudgetUs × SPEAKING_RATE_CPS`, −20% on a "shorter" hint) and `timing_fit.py` (`retranslate` with `fits: false` before rate adjustment) | `tests/test_aws_translation.py`; `tests/test_timing_fit.py` | implemented (M3; the API's retranslate loop on `fits: false` remains informational as in M1) |
...
| FR-020 | `MockSpeechProvider` (local) or `services/media-worker/polycast_worker/providers/aws/polly.py` (`SynthesizeSpeech` PCM 16 kHz + word marks, exact duration from byte length) chosen by `providers/registry.py`; per-locale `CapabilityRecord`s (`beta`, or `unavailable` without a neural voice); `stages/synthesizing.py` writes `speech/<segmentId>.wav`; `apps/api/src/services/jobs.ts` rejects `unavailable` | `tests/test_aws_polly.py`; `tests/test_registry.py`; `apps/api/test/slice.test.ts` | implemented (M1 mock; M3 Polly contract-tested against recorded responses, not yet run against live AWS; second TTS vendor still planned) |
| FR-021 | Default voice table per seed locale in `services/media-worker/polycast_worker/providers/aws/locales.py` (`POLLY_VOICES`, 22 locales, 5 marked unavailable) and `PollyProvider.list_voices()` (`DescribeVoices`); `VoiceAssignment` type in `entities.ts`; audition API will live in `apps/api/src/routes/voices.ts` | `tests/test_aws_polly.py` (table is total over the seed locales) | implemented (M3 voice table + listing); planned (M5 per-speaker assignment UI and audition clips) |
| FR-022 | `services/media-worker/polycast_worker/timing_fit.py` (integer-µs decision: none / rate ±12% / boundary shift ≤ 120 ms / retranslate), `providers/ffmpeg.py::AtempoTimingFitter` (ffmpeg `atempo` chain 0.5–2.0) and `stages/timing.py` writing `speech/<segmentId>.fit.wav` | `tests/test_timing_fit.py`; `tests/test_stage_units.py`; `tests/test_aws_e2e_loop.py` | implemented (M3) |
| FR-023 | `services/media-worker/polycast_worker/providers/ffmpeg.py::DubMixer` (speech at segment start over the bed ducked during dialogue via `volume` + `adelay`/`amix`, two-pass `loudnorm` to −16 LUFS stereo / −19 mono, TP −1 dBTP, verified with `ebur128`) behind `stages/mixing.py`; local mode keeps `PassthroughMixer` | `tests/test_mix_loudness.py` (measured within ±1 LU); `tests/test_aws_e2e_loop.py` | implemented (M3; stem separation FR-007 still absent, the original dialogue is ducked, not removed) |
...
| FR-040 | `services/media-worker/polycast_worker/providers/quality.py::InHouseQualityProvider` over real artefacts: dialogue coverage (> 300 ms → critical), boundary drift (> 120 ms), loudness ± 1 LU, true peak ≤ −1 dBTP, caption timing (2 × 42 chars), entity preservation, frame preservation for video (passes by construction: lip sync not applied); `stages/target_qa.py`. `MockQualityProvider` fixture kept for local mode. Missing: A/V offset, frame diff against a lip-synced render (M4) | `tests/test_quality.py`; `tests/test_aws_e2e_loop.py`; `tests/test_e2e_loop.py` (local fixture); `apps/api/test/slice.test.ts` | implemented (M3 in-house checks; A/V offset and real frame diff planned for M4) |
...
| FR-055 | `Notifier` Protocol in `services/media-worker/polycast_worker/providers/base.py`; `providers/aws/ses.py::SesNotifier` (`SendEmail`, plain text, ids only) and `providers/inapp.py::InAppNotifier` (JSON list under the derived bucket) for review-required / ready / failed / budget-threshold; CLI `python -m polycast_worker.notify`. Not yet called by the API (`apps/api/src/notifications/` is M5) | `tests/test_notify.py` (SES call against a recorded response) | implemented (M3 adapters + CLI); planned (M5 API wiring) |

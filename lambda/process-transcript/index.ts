...
import { PollyClient, StartSpeechSynthesisTaskCommand, Engine, VoiceId, LanguageCode } from '@aws-sdk/client-polly';
...
export async function handler(event: ProcessTranscriptEvent): Promise<ProcessTranscriptResult> {
  ...
  for (const lang of TARGET_LANGUAGES) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (!res.SynthesisTask?.TaskId) throw new Error(`Polly returned no TaskId for ${lang} part ${i + 1}`);
      pollyTasks.push({
  ...
  await putText(
    manifestKey,
    JSON.stringify({ ...result, sourceKey: event.sourceKey, createdAt: new Date().toISOString() }, null, 2),
    'application/json',
  );
  return result;
}
...
async function putText(key: string, body: string, contentType = 'text/plain; charset=utf-8'): Promise<void> {
  await s3.send(new PutObjectCommand({ Bucket: OUTPUT_BUCKET, Key: key, Body: body, ContentType: contentType }));
}
...
export function extractTranscriptText(json: string): string {
  ...
  const text = doc.results?.transcripts?.map((t) => t.transcript ?? '').join(' ').trim();
